...
class PIIDetector:
    def __init__(self):
        # Password ONLY via regex
        self.password_pattern = r"^(?=.*[A-Za-z])(?=.*\d)(?=.*[^A-Za-z0-9]).{6,}$"

    def _load_enabled_pii(self):
        """
        Read detectpii_options.txt EVERY TIME (dynamic config)
        """
        FILE_PATH = "data/detectpii_options.txt"
    ...
    def validate(self, text: str):
        """
        Validate input text for PII
        """

        # Password detection (REGEX ONLY)
        ...
        # Read PII entities dynamically
        enabled_pii = self._load_enabled_pii()
        # print(enabled_pii)

        if "PASSWORD" in enabled_pii :
            if re.fullmatch(self.password_pattern, text):
                return {
                    "passed": False,
                    "message": "PII detected"
                }
        pii_entities = [p for p in enabled_pii if p != "PASSWORD"]
        if pii_entities:
            guard = Guard().use(
                DetectPII(
                    pii_entities=enabled_pii,
